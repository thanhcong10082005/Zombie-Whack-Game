...
class Game:
    def __init__(self):
        ...
        # Load sounds
        self.audio['grasswalk'].play(loops = -1)
        try:
    ...
    def load_assets(self):
        ...
        self.explosion_frames = [pygame.transform.flip(frame, True, False) for frame in import_folder('assets', 'images', 'BoomDie')]
    ...
    def spawn_zombie(self):
        ...
        if current_time - self.last_zombie_spawn >= spawn_interval:
            ...
            if grave_key not in self.last_spawn_per_grave or \
                ...
                # Zombie xuất hiện ngẫu nhiên ở nửa phải màn hình (50%-100%)
                spawn_x = random.randint(int(SCREEN_WIDTH * 0.5), SCREEN_WIDTH + 200)
        ...
        for zombie in self.zombies[:]:
            if zombie.is_clickable() and zombie.get_hit_rect().collidepoint(pos):
                ...
                popup = CartoonPopupText(
                    zombie.rect.centerx, 
                    zombie.rect.centery - 60,  # Position above zombie
                    category, 
                    category_color
                )
    ...
    def run(self):
        ...
        while running:
            ...
            if self.state == "playing":
                if self.health <= 0:
                    self.audio['scream'].play()
                    self.audio['losemusic'].play()
